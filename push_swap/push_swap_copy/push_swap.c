...
/*   Updated: 2022/03/26 16:36:32 by fnochiza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list	*stack_initialization(int argc, char *argv[], int min_a, int max_a)
{
	int		i;
	int		int_value;
	t_list	*list;
...
int	main(int argc, char *argv[])
{
	...
	printf("asdasd\n");
	list_a = stack_initialization(argc, argv, min_a, max_a);
	list_b = create_list_b(list_a);
	//add_first_elem(list_b, 17);
	print_list(list_a, 'a');
	print_list(list_a, 'a');
	while (list_a->size > 2)
	{
		pushing_b(list_a, list_b);
	}
	return (0);
}
